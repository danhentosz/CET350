...
class Ballc extends Canvas implements MouseListener, MouseMotionListener {
	...
	public void updatePhysics() {
		...
		// Initalizes two dummied values, which are used to represent <ball>'s position
		// on the next frame.
		...
		// Check to see if the ball is going to collide with any member of <rects> on
		// the X and Y plane.
		for (Rectangle rect : rects) {
			rect.grow(1, 1);
			...
			rect.contains(new_x, new_y) || rect.contains(new_x + ball.width, new_y + ball.height)
					|| rect.contains(new_x + ball.width, new_y) || rect.contains(new_x, new_y + ball.height) ||

					// Tries to see if the ball contains the rectangle.
					ball.contains(rect.x, rect.y) || ball.contains(rect.x + rect.width, rect.y + rect.height)
					|| ball.contains(rect.x + rect.width, rect.x) || ball.contains(rect.x, rect.y + rect.height)) {
				...
				masks.add(getCollisionDirection(new_x + ball.width, new_y + ball.height, rect.x + rect.width,
						rect.y + rect.height));
