...
public class Program5 {
	/*
	 * The main() method: Description: - Opens a new instance of Bounce().
	 * Preconditions: N/A: method ignores values in <args>[]. Postconditions: -
	 * Creates a new instance of Bounce, <gui>, + from here, implemented methods
...
/*
 * The Bounce() Class: Description: - serves as a container window (Frame()) for
 * an instance of Ball(), - allows interface with Ball() via several UI
 * elements(), + Speed Controls - Scroll (Controls Ball()'s current iteraton
 * speed; higher = faster), + Run / Stop - Button (Toggles between running and
 * pausing Ball()'s iterations), + Circle / Square - Button (Toggles the
 * vector-shape inside of Ball() between a circle and a square), + No Tail /
 * Tail - Button (Toggles whether or not the vector-shape inside of Ball()
 * leaves a drawn tail on the canvas), + Clear - Button (Clears the current
 * canvas inside of Ball()), + Quit Button - Button (Exits the program
 * (functionality is the same as clicking the [x] at the upper right), + Size
 * Controls - Scroll (Controls Ball()'s vector-shape's current size on the
 * screen; higher = larger). - Extends: - Frame(), from java.awt... Implements:
 * - WindowListener() - for window related events (open, close, etc...), -
 * ComponentListener() - for certain java.awt... components (Button(),
 * ScrollBar()), - ActionListener() - for action(s) taken by the user's
 * computer's peripherals, - AdjustmentListener() - for adjustment(s) to certain
 * components (ScrollBar()), - Runnable() - for the use of Thread() throughout
 * instances of GUIBounce(). Preconditions: - N/A. Postconditions: - Constructor
 * returns an instance of GUIBounce(), + internal methods (makeSheet(),
 * initComponents(), sizeScreen(), start()) are also ran. - start() begins an
 * internal loop, which can only be terminated by user input (clicking on "Quit"
 * or [x]).
 */
class Bounce extends Frame implements WindowListener, ComponentListener, ActionListener, AdjustmentListener, Runnable {
	...
	/*
	 * The Bounce() constructor: Description: - calls several subfunctions to
	 * initalize a Bounce() object. Preconditions: N/A. Postconditions: - Returns an
	 * initalized instance of Bounce(), which will be running it's main loop (see
	 * start() and run() for details).
	 */
	public Bounce() {
	...
	/*
	 * The makeSheet() method: Description: - Calculates various mutable values used
	 * for component functionalty and resizing. Preconditions: - N/A.
	 * Postconditions: - Mutates various mutable values inside of Bounce(), and also
	...
	/*
	 * The initComponents() method: Description: - Instantiates all component
	 * objects used by Bounce(), + also adds all components to their relevant
	 * listeners. Preconditions: - Must be called after makeSheet(). Postconditions:
	 * - Populates all object-associated variables with their proper values, and
	 * adds them to implemented listeners of GUIBounce().
	 */
	public void initComponents() throws Exception, IOException {
		...
		// Instantiated after first "validate" so that the "ballPanel" will have already
		// calculated its initial size
	...
	private void addToControlPanel(Component component, int gridx, int gridy,int width,int height) {
		...
		cntrlPanel.add(component,gbc);
	}

	/*
	 * The stop() method: Description: - Serves as a destructor for GUIBounce(), and
	 * a terminator for it's internal loop (see run()). Preconditions: - Can only be
	 * called after initalization. Postconditions: - Frees memory taken up by
	 * GUIBounce()'s member variables and listeners, before returning.
	 */
	public void stop() {
		isRunning = false;
	...
	/*
	 * The run() method: Description: - Iterates so long as <is_running> is true,
	 * and calls <canvObj> depending on whether or not the program is currently
	 * paused. Preconditions: - Can only be called after initalization.
	 * Postconditions: - The program is terminated alongside this function returning
	...
	/*
	 * The adjustmentValueChanged() method: Description: - Listens for
	 * AdjustmentEvents, and changes <speedBar> or <sizeBar> depending on their
	 * origin. Preconditions: - Can only be called after initalization.
	 * Postconditions: - Properly changes the value(s) of either ScrollBar() object.
	 */
	@Override
	...
	/*
	 * The actionPerformed() method: Description: - Listens for ActionEvents, and
	 * alters components of Bounce() depending on their source. Preconditions: - Can
	 * only be called after initalization. Postconditions: - Properly alters
	 * internal variables, labels, or variables of <canvObj> depending on the button
	 * pressed.
	 */
	@Override
	...
	/*
	 * The componentResized() method: Description: - Listens for ComponentEvents,
	 * and handles resizing the screen when they occur. Preconditions: - Can only be
	 * called after initalization. Postconditions: - Calls several methods related
	 * to resizing the screen, before returning.
	 */
	@Override
	public void componentResized(ComponentEvent e) {
		...
		// Check the size and position of the ball and determine if the new width and
		// height are acceptable
		makeSheet();
	}

	/*
	 * The windowClosing() method: Description: - Terminates the program whenever
	 * the windowClosing() window event occurs. Preconditions: - Can only be called
	 * after initalization. Postconditions: - Calls stop(), which assists in
	 * destructing Bounce().
	 */
	@Override
...
class Ballc extends Canvas implements MouseListener, MouseMotionListener {
	...
	private final int BOUNCE_SPEED = 3;
	...
	// Used in "physicsUpdate" to help determine whether the ball moved this frame or not
	...
	private synchronized void tryAddRect() {
		if (dragBox.getWidth() * dragBox.getHeight() <= 2) {
			...
			if (!dragBox.intersects(ball)) {
				// If it will not intersect the ball then check if it is contained within any
				// other rects
	...
	private synchronized void tryRemoveRect(Point point) {
		// Moves backwards through the Vector so that the removal of elements does not
	...
	// Updates the position of the object and checks for collisions with objects
	// If a collision exists, it is resolved by placing the ball outside of the object collided with
	// The distance between the other object in the collision is then subtracted from "leftoverVelocity"
	// If there is any more velocity left-over ("leftoverVelocity"), then this process is repeated
	public void updatePhysics() {
		...
		while (leftoverVel.x != 0 || leftoverVel.y != 0) {
			...
			if (leftoverVel.x != 0) {
				...
				} else { // Else checks for a perimeter collision
					if (ball.getX() + ball.getWidth() + leftoverVel.x > screenWidth - 2) { // Checks the right boundary
						...
						// Places the ball's position at the edge of the wall that it collided with
				...
				// Checks if "leftoverVel" increased (or stayed the same due to being trapped)
			...
			if (leftoverVel.getY() != 0) {
				...
				} else { // Else checks for perimeter collision
					if (ball.getY() + ball.getHeight() + leftoverVel.y > screenHeight - 2) { // Checks the bottom boundary
						// Subtracts the overlap amount from the velocity
	...
	// Returns the position of the first predictable horizontal collision based on 
	// the ball's current position and its horizontal velocity, "horVel"
	private synchronized float getFirstHorizontalCollision(final int horVel) {
		...
		if (horVel > 0) { // The ball is moving right
			for (Rectangle rect : rects) {
				if (rect.getX() > ball.getX() + ball.getWidth() / 2) {
					if (rect.getX() - (ball.getX() + ball.getWidth()) < closestBoundPosition) {
						if (ball.getY() < rect.getY() + rect.getHeight() && ball.getY() + ball.getHeight() > rect.getY()) {
			...
			for (Rectangle rect : rects) {
				if (rect.getX() + rect.getWidth() < ball.getX() + ball.getWidth() / 2) {
					if (ball.getX() - (rect.getX() + rect.getWidth()) < closestBoundPosition) {
						if (ball.getY() < rect.getY() + rect.getHeight() && ball.getY() + ball.getHeight() > rect.getY()) {
	...
	// Returns the position of the first predictable vertical collision based on 
	// the ball's current position and its vertical velocity, "verVel"
	private synchronized float getFirstVerticalCollision(final int verVel) {
		float closestBoundPosition = Float.POSITIVE_INFINITY;
		if (verVel > 0) { // The ball is moving right
			for (Rectangle rect : rects) {
				if (rect.getY() > ball.getY() + ball.getHeight() / 2) {
					if (rect.getY() - (ball.getY() + ball.getHeight()) < closestBoundPosition) {
						if (ball.getX() < rect.getX() + rect.getWidth() && ball.getX() + ball.getWidth() > rect.getX()) {
			...
			for (Rectangle rect : rects) {
				if (rect.getY() + rect.getHeight() < ball.getY() + ball.getHeight() / 2) {
					if (ball.getY() - (rect.getY() + rect.getHeight()) < closestBoundPosition) {
						if (ball.getX() < rect.getX() + rect.getWidth() && ball.getX() + ball.getWidth() > rect.getX()) {
	...
	// Returns true if the boundaries are successfully resized else there was a
	// boundary violation
	...
	// Returns true if the size was updated, and returns false if it was
	// unsuccessful
	public boolean tryUpdateSize(int size) {
		// Repositions the ball so that it may grow about its middle #because the coords
		// now refer to the upper-left corner
		int oldX = ball.x;
		int oldY = ball.y;
		int oldSize = ball.width;

		// Enforces an even numbered size
		ball.height = (size / 2) * 2;
		ball.width = ball.height;
		
		// Repositions the ball so that it appears to grow about the center
